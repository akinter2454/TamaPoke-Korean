...
const SaveField SAVE_FIELDS[] = {
  ...
  { "wgt", SK_U8 },     { "age", SK_U32 },    { "lvmin", SK_U32 },
  { "lvslp", SK_U8 },    { "dexn", SK_I16 },
  ...
  // moves
  { "mvs", SK_BYTES },  { "mvlv", SK_U8 },    { "digmv", SK_U8 },
  // flags
  ...
  // v3.17 game extras: missions, earned items/TMs, research and battle tower
  { "xitem", SK_BYTES }, { "xtm", SK_BYTES }, { "xshb", SK_BOOL },
  { "xmd", SK_U32 }, { "xmk", SK_BYTES }, { "xmp", SK_BYTES }, { "xmc", SK_U8 },
  { "xres", SK_BYTES }, { "xobs", SK_I16 },
  { "xtws", SK_U16 }, { "xtwb", SK_U16 },
  { "xevt", SK_U8 }, { "xerk", SK_U8 }, { "xerid", SK_U8 }, { "xerc", SK_U8 }, { "xelast", SK_U32 },
  // v3.23: type exploration, Battle Tower run buffs, three-slot boss progress
  { "xepa", SK_BOOL }, { "xepr", SK_BOOL }, { "xepm", SK_BOOL }, { "xept", SK_U8 },
  { "xepe", SK_U32 }, { "xepn", SK_U32 }, { "xepk", SK_U8 }, { "xepi", SK_U8 }, { "xepc", SK_U8 }, { "xepw", SK_U8 },
  { "xtbf", SK_BYTES }, { "xtbp", SK_BOOL }, { "xtbc", SK_BYTES },
  { "xbwm", SK_U32 }, { "xbwk", SK_U8 }, { "xbwi", SK_U8 }, { "xbwc", SK_U8 },
  // v3.24: treasure-map hunt and persistent rival progression
  ...
  // completed-but-not-yet-cleared switch, preserving it lets CareSlots recover
  ...
  { "btnsnd", SK_BOOL }, { "btnvol", SK_U8 }, { "btnfix33", SK_BOOL }, { "btnfix37", SK_BOOL },
  { "btnfix38", SK_BOOL }, { "btnfix39", SK_BOOL },
  // Read-only legacy migration keys. Current firmware no longer writes these,
  // but exporting them keeps a just-upgraded old save lossless even before the
  // first normal Pet::save() rewrites the canonical replacements.
  { "gatk", SK_U8 }, { "gdef", SK_U8 }, { "gspe", SK_U8 },
  { "spec", SK_I8 }, { "eggT", SK_I8 },
};
...
#define MAX_VAL 2048       // v3.61 generated research/dex blobs can exceed the old 768-byte box ceiling
...
bool saveImport(const uint8_t *in, size_t n) {
  ...
  if (in[4] < SAVE_MIN_IMPORT_VERSION || in[4] > SAVE_VERSION) return false;
  ...
  // v3.46 also clears the critical-progress shadow: an explicit IMPORT must be
  // authoritative, otherwise an older guard could OR badges/training back into
  // the restored snapshot on the next boot.
  Preferences guard;
  if (guard.begin("tamapoke_guard", false)) { guard.clear(); guard.end(); }
  Preferences p;
