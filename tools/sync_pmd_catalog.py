...
- Include ONLY the 37 user-approved Mega forms that currently have complete
  PMDCollab behaviour sprites, plus five explicitly approved powered forms
  (Primal Kyogre/Groudon, Ultra Necrozma, Crowned Zacian/Zamazenta).
- These approved powered forms are permanent Lv.70 TamaPoke evolutions and independent Dex entries.
...
# v3.89: retain the curated Mega list while adding explicit powered-form evolutions
...
APPROVED_MEGA_NATDEX = {
    ...
    308, 310, 323, 334, 354, 359, 362, 380, 381, 384, 428, 448, 475,
    491, 530, 604, 670, 691, 718, 719, 780, 807, 978,
}
APPROVED_MEGA_COUNT = 37

# Five non-Mega powered forms are intentionally treated exactly like permanent
# level evolutions in TamaPoke. Exact NatDex/form-key pairs prevent future
# PMDCollab gimmick additions from silently entering the evolution graph.
APPROVED_SPECIAL_EVOLUTIONS = {
    (382, '0001'): {'tag':'primal',  'label':'Primal Kyogre',      'ko_mode':'prefix', 'ko_label':'원시'},
    (383, '0001'): {'tag':'primal',  'label':'Primal Groudon',     'ko_mode':'prefix', 'ko_label':'원시'},
    (800, '0003'): {'tag':'ultra',   'label':'Ultra Necrozma',     'ko_mode':'prefix', 'ko_label':'울트라'},
    (888, '0001'): {'tag':'crowned', 'label':'Crowned Sword',      'ko_mode':'suffix', 'ko_label':'왕의 검'},
    (889, '0001'): {'tag':'crowned', 'label':'Crowned Shield',     'ko_mode':'suffix', 'ko_label':'왕의 방패'},
}
APPROVED_SPECIAL_EVOLUTION_COUNT = len(APPROVED_SPECIAL_EVOLUTIONS)
FEATURED_EVOLUTION_NATDEX = {359, 382, 383, 800, 888, 889}
STRICT_BEHAVIOR_ACTIONS = ('Idle','Walk','Attack','Sleep','Hurt')
...
def approved_special_evolution(nat:int, form_key:str, full_name:str=''):
    # Exact National-Dex + PMDCollab form-key identity is the approval gate.
    # Do not rely on tracker display labels: those can be renamed upstream.
    return APPROVED_SPECIAL_EVOLUTIONS.get((int(nat),str(form_key)))
        ...
        for key,sub in sorted((cur.get('subgroups') or {}).items()):
            ...
            if has_sprite(sub) and is_approved_mega(nat,full):
                ...
                out.append({'excluded':False,'natdex':nat,'form_key':'/'.join(new_keys),
                            ...
                            'special_evo':None,'featured_evo':(nat in FEATURED_EVOLUTION_NATDEX),
                            'shiny_required':(nat in FEATURED_EVOLUTION_NATDEX),
                            'strict_behavior':(nat in FEATURED_EVOLUTION_NATDEX)})
...
def direct_forms(nat:int,node:dict):
    ...
    for key,sub in sorted((node.get('subgroups') or {}).items()):
        ...
        special=approved_special_evolution(nat,str(key),full)
        if is_gimmick(full) and not mega and not special:
            ...
            out.append({'excluded':False,'natdex':nat,'form_key':str(key),'pmd_path':chosen_path,
                        ...
                        'special_evo':(special['tag'] if special else None),
                        'featured_evo':(nat in FEATURED_EVOLUTION_NATDEX),
                        'shiny_required':(nat in FEATURED_EVOLUTION_NATDEX),
                        'strict_behavior':(nat in FEATURED_EVOLUTION_NATDEX)})
    return out
...
def main():
    ...
    # The six newly approved evolutions must have a dedicated current shiny.
    if not mega_by_nat[359].get('shiny_path'):
        raise RuntimeError('Mega Absol shiny behavior sprite missing from PMDCollab tracker')
    special_by_key={(int(f['natdex']),str(f['form_key'])):f for f in ordinary_forms if f.get('special_evo')}
    missing_special=sorted(set(APPROVED_SPECIAL_EVOLUTIONS)-set(special_by_key))
    if missing_special:
        raise RuntimeError('approved special evolution sprite missing from PMDCollab tracker: '+repr(missing_special))
    no_shiny=[k for k,f in special_by_key.items() if not f.get('shiny_path')]
    if no_shiny:
        raise RuntimeError('approved special evolution shiny missing from PMDCollab tracker: '+repr(sorted(no_shiny)))
    if len(special_by_key) != APPROVED_SPECIAL_EVOLUTION_COUNT:
        raise RuntimeError(f'approved special evolution count mismatch: {len(special_by_key)} != {APPROVED_SPECIAL_EVOLUTION_COUNT}')
    raw_forms=ordinary_forms+[mega_by_nat[n] for n in sorted(mega_by_nat)]
    form_owners.update(mega_by_nat)
    form_owners.update(n for n,_ in special_by_key)
    ...
    for lk,rec in sorted(lock['forms'].items(),key=lambda kv:int(kv[1]['id'])):
        ...
        form_entries.append({'lock_key':lk,'id':iid,'natdex':nat,'form_key':rec.get('form_key',''),
                             ...
                             'mega':bool((f or {}).get('mega')), 'special_evo':(f or {}).get('special_evo'),
                             'featured_evo':bool((f or {}).get('featured_evo')),
                             'shiny_required':bool((f or {}).get('shiny_required')),
                             'strict_behavior':bool((f or {}).get('strict_behavior'))})
    ...
    for f in form_entries:
        ...
        elif f.get('special_evo'):
            spec=APPROVED_SPECIAL_EVOLUTIONS[(nat,str(f['form_key']))]
            eng=spec['label'].upper()
            ko=(spec['ko_label']+baseko) if spec['ko_mode']=='prefix' else (baseko+' '+spec['ko_label'])
        else:
        ...
        new_entries[iid]={'id':iid,'natdex':nat,'name':eng,'ko_name':ko,'enabled':f['enabled'],'region':f['region'],
                          ...
                          'form_name':f['form_name'],'mega':bool(f.get('mega')),
                          'special_evo':f.get('special_evo'),'featured_evo':bool(f.get('featured_evo')),
                          'shiny_required':bool(f.get('shiny_required')),'strict_behavior':bool(f.get('strict_behavior')),**meta}
        if f['enabled']:enabled_ids.add(iid)
    ...
    for f in form_entries:
        ...
        if f.get('mega'):
            ...
            # Primal / Ultra / Crowned are also permanent collection evolutions.
            add_edge(internal_base_id(nat),iid,SPECIAL_EVOLVE_LEVEL,'approved special evolution')
        elif tag:
            if parent_nat:
    ...
    for (nat,key),iid in FIXED_ALOLA.items():
        ...
        catalog_entries.append({'id':iid,'natdex':nat,'lock_key':f'{nat:04d}/{key}','form_key':key,
                                ...
                                'shiny_path':(f or {}).get('shiny_path'),'name':ko_names[iid],'region':6,'kind':'form','enabled':bool(enabled[iid]),'mega':False,'special_evo':None,'featured_evo':False,'shiny_required':False,'strict_behavior':False})
    for iid,e in sorted(new_entries.items()):
        catalog_entries.append({'id':iid,'natdex':e['natdex'],'lock_key':(f"{e['natdex']:04d}/{e.get('form_key','')}" if e['kind']=='form' else None),
                                ...
                                'special_evo':e.get('special_evo'),'featured_evo':bool(e.get('featured_evo')),
                                'shiny_required':bool(e.get('shiny_required')),'strict_behavior':bool(e.get('strict_behavior')),
    ...
    for e in catalog_entries:
        ...
        audit_entries.append({'id':int(e['id']),'natdex':int(e['natdex']),'name':e.get('name') or str(e['id']),
                              ...
                              'mega':bool(e.get('mega')),'special_evo':e.get('special_evo'),'featured_evo':bool(e.get('featured_evo')),
                              'form_name':e.get('form_name'),'form_key':e.get('form_key'),'lock_key':e.get('lock_key'),
    ...
    special_included=sum(1 for x in catalog_entries if x['enabled'] and x.get('special_evo'))
    ...
    report=[
      'TamaPoke v3.89 current PMDCollab catalog sync - canonical forms + regional evolution + Mega37 + powered evolution 5',
      ...
      f'Approved special evolution entries with real current sprite: {special_included}/{APPROVED_SPECIAL_EVOLUTION_COUNT}',
    ...
    report += ['', 'APPROVED SPECIAL EVOLUTIONS:']
    report += [f"- id {x['id']} nat#{x['natdex']} {x['name']} Lv.{SPECIAL_EVOLVE_LEVEL} ({x.get('pmd_path')})" for x in catalog_entries if x.get('enabled') and x.get('special_evo')]
