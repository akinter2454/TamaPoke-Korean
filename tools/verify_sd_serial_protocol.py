...
assert re.search(r'const\s+EMBEDDED_FIRMWARE\s*=\s*\{.*?\};', html, re.S), 'installer EMBEDDED_FIRMWARE block missing'
assert re.search(r'const\s+EMBEDDED_SPRITE_PAKS\s*=\s*\{.*?\};', html, re.S), 'installer EMBEDDED_SPRITE_PAKS block missing'
