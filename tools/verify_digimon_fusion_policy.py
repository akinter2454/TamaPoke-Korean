...
base=rows[:90]
assert [x[0] for x in base[-4:]]==['Omnimon Alter-S','Chaosmon','Millenniummon','Chaosdramon']
assert all(x[1]=='0' for x in base[-4:])
...
for token in ['best[32]>=55','best[14]>=55','best[66]>=55','best[51]>=55','best[83]>=55','best[48]>=55','i==83&&lv>=60&&a>=80&&d>=60']:
    assert token in cpp, token
assert 'digimonJogressTarget' in cpp and 'digimonJogressTarget' in h
...
print('Digimon fusion/Git policy OK: stable first-90 base, 458-species catalog, legacy save migration, no private sprite payload'+('' if gi else ' (.gitignore absent; content scan used)'))
