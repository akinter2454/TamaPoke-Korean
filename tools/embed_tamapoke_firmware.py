...
s,n=re.subn(r'const\s+EMBEDDED_FIRMWARE\s*=\s*\{.*?\};','const EMBEDDED_FIRMWARE = '+json.dumps(fw,separators=(',',':'))+';',s,count=1,flags=re.S)
if n!=1:raise SystemExit('EMBEDDED_FIRMWARE block not found')
s,n=re.subn(r'const\s+EMBEDDED_SPRITE_PAKS\s*=\s*\{.*?\};','const EMBEDDED_SPRITE_PAKS = {};',s,count=1,flags=re.S)
