...
assert 'static const uint16_t tr[]={0,0,4,8,12,0}' in cpp
...
assert 'static uint8_t dmulBranches' in cpp and 'static uint8_t dmulRoute' in cpp
assert 'const uint16_t tr[4]={a,d,s,h}' in cpp
assert 'uint8_t natural=cur.style&3' in cpp
assert 'Route columns are exactly TamaPoke' in cpp
assert 'total>=25' not in cpp and 'total>=48' not in cpp and 'total>=80' not in cpp
print(f"fixed Digimon branches OK: {checked} legacy sources; DMC/Pendulum preserved; DMUL uses dominant ATK/DEF/SPE/HP")
