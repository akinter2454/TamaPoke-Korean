...
// Append-only blobs must survive both catalog growth and a temporary rollback.
// Preferences::getBytes refuses to copy when the stored value is larger than
// the destination buffer, so read the whole old/new value when necessary and
// preserve the common prefix. The tail stays zero-initialised.
static void loadPrefixBlob(Preferences &p, const char *key, void *dst, size_t cap) {
  size_t stored = p.getBytesLength(key);
  if (!stored) return;
  memset(dst, 0, cap);
  if (stored <= cap) {
    p.getBytes(key, dst, cap);
    return;
  }
  uint8_t *tmp = (uint8_t *)malloc(stored);
  if (!tmp) return;
  if (p.getBytes(key, tmp, stored) == stored) memcpy(dst, tmp, cap);
  free(tmp);
}
...
void GameExtras::begin() {
  ...
  // Research is indexed by species ID and therefore grows with DEX_COUNT.
  // A shorter old backup keeps its existing species scores at the front; newly
  // added species start at zero. A rollback keeps the prefix it understands.
  loadPrefixBlob(prefs, "xres", _research, sizeof(_research));
