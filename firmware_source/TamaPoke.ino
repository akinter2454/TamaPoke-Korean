...
//
// Verified by the user's DMC 1~5 preview:
//   IDLE  0 <-> 1
//   WALK  0 <-> 1, original art faces left; mirror only while moving right
//   EAT   9 <-> 8
//   REFUSE 10 with horizontal flip
//
// The remaining roles use the matching COLOR action frames and are shared by
// DMC, Pendulum and DMUL so all Digimon can behave like the Pokemon actor
// without changing DGI3 or copying PMD image resources.
enum DigiMotionRole : uint8_t {
  DIGI_MOTION_IDLE = 0,
  DIGI_MOTION_WALK,
  DIGI_MOTION_EAT,
  DIGI_MOTION_SLEEP,
...
static uint8_t digiMotionFrame(uint8_t role,uint32_t elapsed){
  switch(role){
    case DIGI_MOTION_IDLE:   return (elapsed/450u)&1u;              // 0,1
    case DIGI_MOTION_WALK:   return (elapsed/320u)&1u;              // 0,1
    case DIGI_MOTION_EAT:    return ((elapsed/260u)&1u)?8u:9u;      // 9,8
    case DIGI_MOTION_SLEEP:  return 11u+((elapsed/850u)&1u);        // 11,12
    case DIGI_MOTION_HURT:   return 13u+((elapsed/320u)&1u);        // 13,14; battle impact + jitter
    case DIGI_MOTION_SICK:   return 13u+((elapsed/450u)&1u);        // 13,14; home unmet-care state, no jitter
    case DIGI_MOTION_ATTACK: return 6u+((elapsed/300u)&1u);         // 6,7
    case DIGI_MOTION_POSE:   return 1u+((elapsed/300u)&1u);         // 1,2
    case DIGI_MOTION_REFUSE: return 10u;
    default: return 0u;
  }
}

static void digiBehNext();  // defined after global beh state
...
// Same high-level home-screen scheduler as Pokemon:
// walk ~35%, expressive pose ~25%, otherwise idle.
static void digiBehNext(){
  uint32_t now=millis();
  beh.t0=now;
  int r=random(100);
  if(r<35){
...
// - while evolving: fixed DGI frame 2 (no 1<->2 sprite wobble)
// - after evolution completes: play the existing Pose 1<->2 celebration briefly
static bool digiWasEvolving = false;
static uint32_t digiEvolveCelebrateUntil = 0;
...
static int8_t btlDigiFrame[2]={-1,-1};
static uint16_t btlDigiTransparent[2]={0,0};
static bool loadBattleDigi(uint8_t who,uint16_t id,uint8_t frame);
...
void loop() {
  ...
  static bool wasEvoReady = false, wasJogressReady = false;
  bool evoReady = pet.wantEvolveButton();
  bool jogressReady = pet.canJogressNow();
  if ((evoReady && !wasEvoReady || jogressReady && !wasJogressReady) && !screenOff) sfxPlay(SFX_MEDAL);
  wasEvoReady = evoReady;
  wasJogressReady = jogressReady;
...
void handleSerial() {
  ...
  if (line.startsWith("DIGI START ")) {
    ...
    Serial.println("DONE");
  } else if (line == "DIGI JOGRESS") {
    bool ok=digiPet.jogress();
    Serial.printf("DIGI %s %s Lv.%u\n",ok?"JOGRESSED":"WAIT",digiPet.species().name,digiPet.level());
    Serial.println("DONE");
    ...
    static uint8_t buf[SAVE_SERIAL_CAP];
    ...
    static uint8_t in[SAVE_SERIAL_CAP];
    static size_t inN = 0;
...
void onTap(int16_t x, int16_t y) {
    ...
    if (!starterRegionDone) {
      ...
      if (r >= 0) {
        if (r >= GAL_REGIONS) {
          ...
          pet.setDigimonVersion(digimonVersionFromSlot(slot));
  ...
  if (cardOpen) {
    if (choiceKind == 1 || choiceKind == 4) {
      ...
      bool actionYes = inX && y >= c1t && y <= c1b;
      bool actionNo = inX && y >= c2t && y <= c2b;
      if (actionYes) {
        int16_t old = pet.speciesId;
        bool wasDigimon = pet.currentIsDigimon();
        if(choiceKind==4) pet.jogress(); else pet.evolve();
        if (wasDigimon) {
        ...
        cardOpen = false;
      } else if (actionNo && choiceKind==1) {
        pet.declineEvolve();
      }
      if (actionYes || actionNo) choiceKind = 0;
      return;
    }
  ...
  if (choiceKind) {          // dialogo de decision: boton accion (arriba) / mantener (abajo)
    ...
    if (choiceKind == 1) {                 // normal evolution
      if (b1) {
        int16_t old = pet.speciesId;
        bool wasDigimon = pet.currentIsDigimon();
        pet.evolve();
    ...
    } else if (choiceKind == 4) {          // explicit Jogress
      if (b1) { pet.jogress(); if (evoPmd.loaded) evoPmd.unload(); }
...
void renderCardStats() {
  ...
  if(normalReady && jogressReady){
    ...
    gfx->drawRoundRect(42,CARD_EVO_Y,182,CARD_EVO_H,12,UI_INK);
    gfx->setTextColor(UI_WHITE);uiDrawCenteredFit("일반 진화",133,CARD_EVO_Y+14,164,2,1);
    gfx->fillRoundRect(242,CARD_EVO_Y,182,CARD_EVO_H,12,UI_BAR_OK);
    gfx->drawRoundRect(242,CARD_EVO_Y,182,CARD_EVO_H,12,UI_INK);
    gfx->setTextColor(UI_WHITE);uiDrawCenteredFit("조그레스",333,CARD_EVO_Y+14,164,2,1);
  }else if(normalReady){
    ...
    gfx->setTextColor(UI_WHITE);uiDrawCenteredFit("진화하기",CX,CARD_EVO_Y+12,CARD_EVO_W-24,3,2);
  }else if(jogressReady){
...
static void btlFreeSprites() {
  for (int i = 0; i < 2; i++) { btlPmd[i].unload(); btlPmdDex[i] = 0; btlDigiFor[i] = -1; btlDigiFrame[i] = -1; }
}
...
static void btlSide(int tx, int ty, int sx, int sy, const Combatant &c, uint8_t who) {
  ...
  if(isDigimonId(c.dex)){
    uint8_t df;
    if(now<btlHitUntil[who])
      df=digiMotionFrame(DIGI_MOTION_HURT,now-(btlHitUntil[who]-BTL_HIT_MS));
    else if(now<btlLungeUntil[who])
      df=digiMotionFrame(DIGI_MOTION_ATTACK,now-(btlLungeUntil[who]-BTL_LUNGE_MS));
    else
      df=digiMotionFrame(DIGI_MOTION_IDLE,now);

    if(loadBattleDigi(who,digimonIndex(c.dex),df)){
      int scale=2,dx=sx+ox,dy=sy+oy;
      // COLOR/DGI source art faces left. Player stands on the left and must
      // face right (mirrored); foe stands on the right and uses source facing.
      bool mirror=who==0;
...
static int eggRegionTap(int16_t x, int16_t y) {
  ...
  if (hit) {
    if(pet.eggSource>=REGION_COUNT+DIGI_DEVICE_SLOT_COUNT-1)pet.setRegion(0);
    else if(pet.eggSource>=REGION_COUNT){uint8_t slot=pet.eggSource-REGION_COUNT+1;pet.setDigimonVersion(digimonVersionFromSlot(slot));}
...
uint8_t rpickRegions(uint8_t mode) {
  return (mode == RPICK_FOR_GYMS) ? (uint8_t)GYM_REGIONS :
         (mode == RPICK_FOR_START ? (uint8_t)(GAL_REGIONS+DIGI_DEVICE_SLOT_COUNT) : (uint8_t)GAL_REGIONS);
}
...
static bool loadBattleDigi(uint8_t who,uint16_t id,uint8_t wantedFrame){
  ...
  char path[32];
  snprintf(path,sizeof(path),"/digimon/d%03u.dgi",id);
  File f=SD_MMC.open(path,FILE_READ);
  if(!f)return false;

  uint8_t h[10];
  if(f.read(h,10)!=10){f.close();return false;}
  bool old=!memcmp(h,"DGI1",4)||!memcmp(h,"DGI2",4),v3=!memcmp(h,"DGI3",4);
  size_t bytes=48*48*2;
  if((!old&&!v3)||h[4]!=(uint8_t)id||h[8]!=48||h[9]!=48||!h[7]||
     f.size()!=10+(size_t)h[7]*bytes){
    f.close();return false;
  }

  uint8_t frame=wantedFrame%h[7];
  if(!f.seek(10+(size_t)frame*bytes)||
     f.read((uint8_t*)btlDigiPixels[who],bytes)!=(int)bytes){
    f.close();return false;
  }
  f.close();

  btlDigiFor[who]=id;
  btlDigiFrame[who]=wantedFrame;
  btlDigiTransparent[who]=v3?0x0001:0;
  return true;
}
...
static void digiDexNumber(uint16_t id,char*out,size_t n){
  ...
  else if(isDmulVersion(DIGI_SPECIES[id].version))snprintf(out,n,"U%u-%02u",DIGI_SPECIES[id].version-16,digiLocalIndex(id));
...
static const char*digiDexRule(uint16_t id){
  ...
  if(id==DIGI_CHAOSDRAMON)return "무겐드라몬 Lv.60 + 공격 80 + 방어 60";
  {
    ...
    if(!strcmp(n,"Dukemon Crimson Mode"))return "듀크몬 Lv.60 + 공격/스피드 합 80";
    if(!strcmp(n,"Examon"))return "슬레이어드라몬 + 브레이크드라몬 각 Lv.55 조그레스";
    if(!strcmp(n,"Armagemon"))return "디아블로몬 Lv.60 + 공격 80 + 쿠라몬 육성 기록";
    if(!strcmp(n,"GraceNovamon"))return "디아나몬 + 아폴로몬 각 Lv.55 조그레스";
    if(!strcmp(n,"MirageGaogamon Burst Mode"))return "미라쥬가오가몬 Lv.60 + 공격/스피드 합 90";
    if(!strcmp(n,"Beelzebumon Blast Mode"))return "베르제브몬 Lv.60 + 공격/스피드 합 90 + 바알몬 Lv.45 기록";
    if(!strcmp(n,"JESmon GX"))return "제스몬 Lv.60 + 간쿠몬 Lv.55 육성 기록";
    if(!strcmp(n,"Lucemon Falldown Mode"))return "루체몬 Lv.45 + 총 훈련 70";
...
static void drawDigiSprite() {
  ...
  uint8_t frame=0;
  bool flip=false;
  int16_t x=(int16_t)beh.x;
  PetMood mood=pet.mood();

  // Pokemon-style priority: sleep/eat/hurt state overrides ambient behavior.
  if(mood==MOOD_SLEEPING){
    beh.mode=0;
    frame=digiMotionFrame(DIGI_MOTION_SLEEP,now);
  }else if(mood==MOOD_EATING){
    beh.mode=0;
    frame=digiMotionFrame(DIGI_MOTION_EAT,now);
  }else if(mood==MOOD_SAD){
    // Same source sprites as Hurt (13/14), but this is the calm home "Sick"
    // state: no battle jitter and a slower cadence like the other care loops.
    // Pet::mood() reaches MOOD_SAD when ANY of fullness/joy/energy/hygiene
    // falls below the existing care threshold (lowestStat() < 25).
    beh.mode=0;
    frame=digiMotionFrame(DIGI_MOTION_SICK,now);
  }else if((int32_t)(digiActionUntil-now)>0){
    // Touch and post-training reactions use the same expressive role that
    // Pokemon uses for a short pose/gesture rather than inventing a new action.
    frame=digiMotionFrame(DIGI_MOTION_POSE,now);
  }else{
    if(now>beh.until)digiBehNext();
    ...
    if(beh.mode==1){
      ...
      if(fabsf(d)<4.0f){
        digiBehNext();
        x=(int16_t)beh.x;
        frame=digiMotionFrame(DIGI_MOTION_IDLE,now);
      }else{
        bool movingRight=d>0;
        beh.x+=(movingRight?3.0f:-3.0f);
        x=(int16_t)beh.x;
        frame=digiMotionFrame(DIGI_MOTION_WALK,now-beh.t0);

        // User-verified COLOR orientation: the source art faces LEFT.
        // Left travel uses the source pixels; right travel mirrors them.
        flip=movingRight;
      }
    }else if(beh.mode==2){
      frame=digiMotionFrame(DIGI_MOTION_POSE,now-beh.t0);
    }else{
      frame=digiMotionFrame(DIGI_MOTION_IDLE,now);
    }
  }
  ...
  // Legacy 3-frame DGI1/DGI3 packs remain readable. Their old simple looping
  // fallback is preserved rather than forcing 15-frame action numbers on them.
  if(digiFrameCount<15){
    frame=(now/420u)%digiFrameCount;
    if(!loadDigiSprite(spriteId,frame))return;
    if(beh.mode!=1){x=CX;flip=false;}
  }
...
void renderDigi(){
  ...
  if(!digiPet.enabled){gfx->setTextColor(UI_INK);uiDrawCenteredFit("버전을 선택해 디지타마를 받으세요",CX,196,410,2,1);}
    ...
    bool de=digiPet.canEvolve(), dj=digiPet.canJogress();
    if(de&&dj){
      gfx->fillRoundRect(151,390,126,44,12,UI_BAR_BAD);gfx->drawRoundRect(151,390,126,44,12,UI_INK);uiDrawCenteredFit("일반 진화",214,404,112,1,1);
      gfx->fillRoundRect(290,390,126,44,12,UI_BAR_OK);gfx->drawRoundRect(290,390,126,44,12,UI_INK);uiDrawCenteredFit("조그레스",353,404,112,1,1);
    }else{
      bool ready=de||dj;const char*lab=de?"진화하기":(dj?"조그레스":"진화 조건 미달");
      gfx->fillRoundRect(151,390,265,44,12,ready?UI_BAR_OK:UI_TRACK);gfx->drawRoundRect(151,390,265,44,12,UI_INK);uiDrawCenteredFit(lab,283,402,250,2,1);
    }
  }
...
void digiTap(int16_t x,int16_t y){
  ...
  if(y>=390&&y<=434&&x>=151){
    bool de=digiPet.canEvolve(),dj=digiPet.canJogress(),ok=false;
    if(de&&dj){if(x<=277)ok=digiPet.evolve();else if(x>=290)ok=digiPet.jogress();}
    else if(de)ok=digiPet.evolve(); else if(dj)ok=digiPet.jogress();
    sfxPlay(ok?SFX_MEDAL:SFX_DENY);digiLoaded=-1;digiLoadedFrame=-1;return;
  }
...
void renderGallery() {
  if (galleryDetail) {  // vista detalle: se redibuja siempre (animada)
    ...
    if (pet.huntTargetDex() >= 1) {
      ...
      int16_t hd = pet.huntTargetDex();
      if (pet.isRegistered(hd))
        snprintf(curHunt, sizeof(curHunt), "현재 찾기: N.%03d %s  %u/6", hd, localizedSpeciesName(hd), pet.huntMisses());
      else
        snprintf(curHunt, sizeof(curHunt), "현재 찾기: N.%03d  %u/6", hd, pet.huntMisses());
      uiDrawCenteredFit(curHunt, CX, 362, 420, 1, 1);
    }
    ...
    uiSetCursor(CX - uiTextHalfWidth(rs, 2), 388);
    gfx->print(rs);
    gfx->setTextColor(UI_TRACK);
  ...
  for (int r = 0; r < 4; r++) {
    for (int c = 0; c < 4; c++) {
      ...
      if (pet.huntTargetDex() == dex) {
        // High-contrast hunt marker: the old 1px pale-yellow text disappeared
        // on the bright round-panel background of the physical AMOLED.
        gfx->drawRoundRect(x + 1, y + 1, GAL_CELL - 2, GAL_CELL - 2, 8, UI_BAR_WARN);
        gfx->drawRoundRect(x + 2, y + 2, GAL_CELL - 4, GAL_CELL - 4, 7, UI_BAR_WARN);
        gfx->fillRoundRect(x + 8, y + 57, GAL_CELL - 16, 19, 5, UI_BAR_WARN);
        gfx->setTextColor(UI_INK);
        uiDrawCenteredFit("찾는 중", x + GAL_CELL/2, y + 62, GAL_CELL - 20, 1, 1);
      }
    }
  }
  ...
  if (pet.huntTargetDex() >= 1) {
    ...
    int16_t hd = pet.huntTargetDex();
    if (pet.isRegistered(hd))
      snprintf(huntFoot, sizeof(huntFoot), "찾는 포켓몬: %s  %u/6", localizedSpeciesName(hd), pet.huntMisses());
    else
      snprintf(huntFoot, sizeof(huntFoot), "찾는 포켓몬: N.%03d  %u/6", hd, pet.huntMisses());
    // Solid warning chip + dark text remains readable on the actual AMOLED.
    gfx->fillRoundRect(62, 392, 342, 30, 9, UI_BAR_WARN);
    gfx->drawRoundRect(62, 392, 342, 30, 9, UI_INK);
    gfx->setTextColor(UI_INK);
    uiDrawCenteredFit(huntFoot, CX, 400, 326, 2, 1);
  }
...
static void drawDigiCeremony(){
  ...
  if(panic){
    ...
    if(t<0.30f){
      frame=digiMotionFrame(DIGI_MOTION_HURT,now);
      x=CX+(int)(4*sinf(now*0.04f));
    }else{
      x=CX-(int)(((t-0.30f)/0.70f)*(CX+120));
      frame=digiMotionFrame(DIGI_MOTION_WALK,now);
      // Walking left: source art already faces left, so do not mirror.
      flip=false;
  ...
  if(t<0.45f)frame=digiMotionFrame(DIGI_MOTION_POSE,now);
  else{
...
// Digimon keep the same halo/rays/sparks/timing as Pokemon evolution, but
...
void drawPet() {
  if (pet.currentIsDigimon()) {
    uint32_t now=millis();
    if (pet.evolving()) {
      digiWasEvolving=true;
      drawDigiEvolveFX(now);
    } else {
      if (digiWasEvolving) {
        digiWasEvolving=false;
        digiEvolveCelebrateUntil=now+1800;
      }
...
void drawBath() {
  ...
  if (now > bathUntil) {
    ...
    if (bathPending) {
      ...
      if (pet.currentIsDigimon()) {
        digiReact(DIGI_ANIM_TOUCH, 1800);
      } else if (pmd.has(PMD_POSE)) {
        beh.mode = 2;
        beh.act = PMD_POSE;
        beh.t0 = now;
