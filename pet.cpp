...
static uint16_t petDigimonEggSpecies(uint8_t version) {
  uint16_t first=0; uint8_t count=0;
  const uint8_t wanted=DIGI_BABY1;
  for(uint16_t i=0;i<DIGI_SPECIES_COUNT;i++)if(DIGI_SPECIES[i].version==version&&DIGI_SPECIES[i].stage==wanted){if(!count)first=i;count++;}
  if(!count)return 0;
  return first; // every DMUL field slot owns one selected Baby I DigiTama
}
...
void Pet::newEgg() {
  ...
  huntRollHit = false;
  if (eggSource >= REGION_COUNT && eggSource < REGION_COUNT + DIGI_DEVICE_SLOT_COUNT) {
    uint8_t slot=eggSource-REGION_COUNT;
    uint8_t version=digimonVersionFromSlot(slot);
...
void Pet::setDigimonVersion(uint8_t version) {
  if (!isDigimonDeviceVersion(version)) return;
  uint8_t slot=digimonSlotFromVersion(version);
...
uint16_t Pet::jogressTarget() const {
  if (frozen || isEgg() || sleeping || ceremony != CER_NONE || !currentIsDigimon()) return DIGI_NO_FUSION;
  uint16_t i=digimonIndex(speciesId);
  return digimonJogressTarget(i,level(),trAtk,trDef,trSpe,trHp,digiBest);
}

bool Pet::canJogressNow() const {
  uint16_t t=jogressTarget();
  return t!=DIGI_NO_FUSION && t!=digimonIndex(speciesId);
}

bool Pet::jogress() {
  uint16_t next=jogressTarget();
  if(next==DIGI_NO_FUSION || next==digimonIndex(speciesId)) return false;
  uint16_t old=digimonIndex(speciesId);
  if(level()>digiBest[old]) digiBest[old]=level();
  prevSpeciesId=speciesId;
  speciesId=makeDigimonId(next);
  evoDeclinedLv=0;
  registerSpecies(speciesId);
...
void Pet::save() {
  ...
  // Capacity is deliberately much larger than the current roster. Persist only
  // the active catalog prefix so raising the reserve to 2048 does not bloat NVS
...
void Pet::load() {
  ...
  if (eggSource >= REGION_COUNT + DIGI_DEVICE_SLOT_COUNT) eggSource = region;
